...
- Read `mem:tech_stack` for language/build/tooling details. Read `mem:conventions` for repo-specific editing rules. Read `mem:tooling/serena` for repo-specific notes on Serena's strengths and limits with Nix modules and Lua plugin specs. Read `mem:suggested_commands` for commands. Read `mem:task_completion` before claiming task completion.