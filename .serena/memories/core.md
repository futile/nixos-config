...
- Read `mem:tech_stack` for language/build/tooling details. Read `mem:conventions` for repo-specific editing rules. Read `mem:suggested_commands` for commands. Read `mem:task_completion` before claiming task completion.