{ config, pkgs, thisFlakePath, ... }: {
  ...
  xdg = {
    enable = true;
    configFile."nushell/shared_config.nu".source = config.lib.file.mkOutOfStoreSymlink
      "${thisFlakePath}/dotfiles/nushell/shared_config.nu";
    configFile."nushell/shared_env.nu".source = config.lib.file.mkOutOfStoreSymlink
      "${thisFlakePath}/dotfiles/nushell/shared_env.nu";
  };
