...
- Follow the current session's exposed tool schemas. With infinite-context v2, `context_map` lists visible roots or one fold's direct children using 1-based `offset`/`limit` pages. `context_fold` wraps disjoint contiguous visible-root ranges with an explicit `summary` (empty is allowed); existing folds become unchanged children.
