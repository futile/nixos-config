...
- Use `context_fold`/`context_unfold` to maintain completed or superseded context. `context_search` takes direct JavaScript regular-expression `patterns`; escape metacharacters when seeking literal text, and let invalid-pattern errors surface. Fold stubs have no inline IDs; obtain IDs from `context_map` and fold/unfold tool results.
